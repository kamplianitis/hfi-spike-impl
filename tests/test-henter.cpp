...
// bool display(uint64_t options){
//     switch(options){
//         case 0:
//             if (hfi_inspect(0) == 0){
//                 std::cout << "Not in HFI Sandbox" << std::endl;
//                 return false;
//             }
//             std::cout << "In HFI Sandbox" << std::endl;
//             return true;
//         default:
//             uint64_t lower = hfi_inspect(1);
//             uint64_t upper = hfi_inspect(2);
//             std::cout << "HFI Sandbox Bounds: [" << std::hex << lower << ", " << upper << ")" << std::dec << std::endl;
//             return true;
//     }
// }
...
{   
    ...
    // hfi_set_permissions(0);
    // uint64_t perm = hfi_get_permissions();
    // std::cout << "permissions set " << std::hex << perm << std::dec << std::endl;
    hfi_enter(0);   
    hfi_exit();

    uint64_t FULL_PERM = 0xF; // full perm for explicit data region 1

    uint64_t array[] = {1, 2, 3, 4, 5, 6, 7, 8};
    uint64_t options = 0;

    std::cout << "array address: " << std::hex<< (uint64_t)&array[0] << std::dec << std::endl;
    ...
    hfi_set_permissions(0b01111111); // no read perm for implicit region 1
    uint64_t perm = hfi_get_permissions();
    std::cout << "permissions set " << std::hex << perm << std::dec << std::endl;
